...
import { adminDeleteKarigar, adminGetAllKarigar } from "../../../actions";
...
const range = len => {
  ...
  const newPerson = (index,kdata,deleteKarigarReq) => {
 ...
 function makeData(kdata,deleteKarigarReq,...lens) {
    ...
    const makeDataLevel = (depth = 0) => {
      ...
      return range(len).map(d => {
        ...
        return {
          ...newPerson(d,kdata,deleteKarigarReq),
...
function AdminKarigarTable() {
  ...
  let kdata=karigars.data.karigar;

  const deleteKarigarReq=(id)=>{
    setOrderDeleteId(id);
    setViewModal(true);
  }
...
export default AdminKarigarTable;
