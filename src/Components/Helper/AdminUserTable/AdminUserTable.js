...
import {  adminDeleteuser, adminGetAllUser, adminUActivateDeactivate, adminUDeactivate } from "../../../actions";
...
const range = len => {
  ...
  const newPerson = (index,udata,deleteUserReq,handleUActivate) => {
    ...
    return {
      ...
      icon:<button  onClick={() => deleteUserReq(udata[index]._id)}  className='adkarigar-btn del-icon'><HiOutlineTrash id='deleteicon'/></button>
    }
  }
 function makeData(udata,deleteUserReq,handleUActivate,...lens) {
    //pass total data as lens
    const makeDataLevel = (depth = 0) => {
      const len = lens[depth]
      //depth = 0 means lens[0]=>total data in table

      // range will return arr=>[0,1,2,3,4...50(total data)];
      return range(len).map(d => {
        // from observation lens[0] is set all other index of lens is not set => undefined
        return {
          ...newPerson(d,udata,deleteUserReq,handleUActivate),
...
function AdminUserTable() {
  ...
  let udata=users.data.user;
  const handleUActivate = (userId,index,udata)=>{
    if(udata[index].isActive){
      setuserID(userId);
      dispatch(adminUDeactivate({userId})).then(()=>{
        if (users.success) {
        }
    })
      dispatch(adminGetAllUser());
